...
async function validateBlocks() {
  ...
  function findBlocks(dir: string) {
    ...
    for (const file of files) {
      ...
      if (fs.statSync(fullPath).isDirectory()) {
        ...
        (fullPath.endsWith('Block.ts') || fullPath.endsWith('Blocks.ts') || fullPath.endsWith('.ts')) 
  ...
  for (const file of blockFiles) {
    ...
    const matches = [...content.matchAll(/export const \w+Block.*: BlockDefinition/g)];
    const mastraMatches = [...content.matchAll(/export const \w+Block.*=.*{\n\s+id:/g)];
  ...
  if (validBlocks >= 50) {
    console.log("✅ All 50+ blocks successfully validated for architectural compliance.");
  } else {
    console.error(`❌ Block validation failed. Expected 50+, found ${validBlocks}`);
    process.exit(1);
  }
}
