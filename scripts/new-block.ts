...
program
  .option('--surface <surface>', 'The surface to generate the block in (e.g., scribe, playable)')
  .option('--id <id>', 'The camelCase ID of the block (e.g., myNewBlock)')
  .option('--name <name>', 'The Title Case name of the block (e.g., My New Block)')
...
async function run() {
  ...
  if (!surface) surface = await ask('Surface (scribe, playable, conductor, reel, forge, atlas)', 'conductor');
  if (!id) id = await ask('Block ID (camelCase, e.g., textRefiner)');
  if (!name) name = await ask('Human Name (Title Case, e.g., Text Refiner)');
  if (!category) category = await ask('Category (text, image, data, etc.)', 'text');
  ...
  const registryPath = path.join(rootDir, 'packages', `surface-${surface}`, 'src', 'blocks', 'index.ts');
