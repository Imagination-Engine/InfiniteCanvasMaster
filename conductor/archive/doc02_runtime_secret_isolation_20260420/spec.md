...
- **Ephemeral State:** The decrypted credentials must _never_ be logged, stored on disk, or returned to the client frontend.
...
- Actually implementing the specific OAuth2 refresh flows for each external provider; this track only builds the _foundation_ that those integrations will plug into.
