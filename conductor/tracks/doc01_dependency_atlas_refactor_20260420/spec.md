...
- **Lazy Execution:** Do *not* install these dependencies into the `package.json` files yet; the respective surface tracks will handle installation when required. The Atlas serves as the strict blueprint.
...
## 4. Out of Scope
- Actually modifying `package.json` files or installing node modules; this track establishes the *governance* over them.