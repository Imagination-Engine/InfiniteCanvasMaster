...
- **Imagination Canvas Kit (`@iem/imagination-canvas-kit`):** A sovereign, production-grade spatial engine implementing a **Rich Semantic Object Model**, foundational state management (Zustand) optimized with **granular state selectors for elite drag performance**, a modular shell architecture, a specialized Viewport Engine, a high-fidelity **Selection & Transformation Engine**, robust **Native Interactions** (creation via high-precision library-to-canvas drop), **Rich Embedded Content** blocks equipped with deep event isolation, advanced **Spatial Organization Algorithms**, a **Framer Motion Choreography Engine** enabling seamless, state-preserving expansion transitions, the **Spatial Agent Interaction Layer** comprising the `agentTaskStore`, **Mutation Preview** hooks, and **Streaming Choreography** components, the **History & Persistence Engine** (`useHistoryStore`, `useAutosave`) for transactional Undo/Redo and snapshotting, the **Collaboration & Presence Engine** managing semantic spatial comments and multi-actor cursor/viewport syncing, a comprehensive **Touch Gesture Engine** optimizing interactions and hit targets for mobile and tablet parity, and a standardized **Semantic Design Token Map** managing density modes, motion fallbacks, and agent status stylings, and a **Dynamic Block Registry** with **Semantic Zoom** performance optimizations.
- **Custom Library Engine:** Orchestrated via `useLibraryStore`, providing a persistence pipeline for user-configured blocks. It bridges the transient canvas state to the permanent relational database, enabling a growing ecosystem of specialized agentic tools. The kit now includes a **Global Block Icon Map** using Lucide primitives and a refined **SVG Connector Engine** supporting directional flow markers and real-time execution pulse animations.
...
- **Agent Factory:** A specialized service in `@iem/agents` that dynamically instantiates Mastra agents based on block definitions, injecting sovereign personas, specialized instructions, and tool bindings on-the-fly.
