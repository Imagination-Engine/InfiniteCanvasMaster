...
import { Bot, Minimize2, Maximize2, X, Sparkles, Terminal, ChevronRight } from "lucide-react";
...
const ThinkingBubble = () => (
  <div className="flex items-center gap-2 text-brand-purple/50 animate-pulse py-2">
    <Terminal size={14} />
    <span className="text-[10px] font-black uppercase tracking-widest italic">Neural Engine Processing...</span>
  </div>
);
...
export const ChatShell: React.FC<ChatShellProps> = ({
  ...
  const { messages, input, handleInputChange, handleSubmit, error, append, status } =
    useChat({
      api: "/api/chat",
    ...
    <div className={containerClasses}>
      ...
      {fullScreen && (
        <div className="flex flex-col items-center py-12 space-y-4">
           ...
           <h2 className="text-2xl font-black uppercase tracking-[0.2em] text-white">Neural Interface</h2>
           <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-brand-text-muted">Imagination Engine v1.0.0</p>
        </div>
      )}
          ...
          >
            ...
            {messages.map((m: any) => {
              ...
              return (
                ...
                >
                  {/* Avatar / Label */}
                  <div className={`flex items-center gap-2 mb-1 px-2 ${!isAssistant ? 'flex-row-reverse' : ''}`}>
                  ...
                  >
                    ...
                    {m.toolInvocations?.map((toolInvocation: any) => {
                      ...
                      if (toolName === 'generate_canvas_blueprint') {
                        ...
                        return (
                          <div key={toolCallId} className="mt-6 p-5 rounded-2xl bg-black/40 border border-white/5 backdrop-blur-xl group/tool transition-all hover:border-brand-purple/30">
                            <div className="flex items-center justify-between mb-3">
                               <div className="flex items-center gap-2">
                                  <div className="w-2 h-2 rounded-full bg-brand-purple animate-pulse" />
                                  <span className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-purple">Blueprint Synthesis</span>
                               </div>
                               <Sparkles size={12} className="text-brand-purple opacity-50 group-hover/tool:opacity-100 transition-opacity" />
                            </div>
                            <h4 className="text-white font-bold text-sm mb-1">{args.blueprint_name}</h4>
                            <p className="text-[11px] text-brand-text-muted leading-relaxed mb-3">{args.description}</p>
                            
                            <div className="flex gap-2 mb-5 flex-wrap">
                               <span className="px-2 py-1 rounded bg-white/5 border border-white/10 text-[9px] text-white/50">{args.nodes?.length || 0} Nodes</span>
                               <span className="px-2 py-1 rounded bg-white/5 border border-white/10 text-[9px] text-white/50">{args.edges?.length || 0} Edges</span>
                            </div>

                            <button 
                              onClick={() => {
                                window.dispatchEvent(new CustomEvent('iem:apply-blueprint', { detail: args }));
                              }}
          ...
          >
            <div className="relative flex items-center max-w-2xl mx-auto w-full group">
              <input
                ...
                placeholder={fullScreen ? "Manifest your intent..." : "Ask the engine..."}
