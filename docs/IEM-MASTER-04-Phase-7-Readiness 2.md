...
4. **Technical Extrapolation (The AI PM Role):** The AI acts as a Technical Product Manager. It takes the human feedback and extrapolates the _entire blast radius_ of the required changes.
   - _Example:_ "Make the chat load faster" -> Extrapolates to: UI loading states + API latency optimization + Database indexing + Mastra memory caching.
