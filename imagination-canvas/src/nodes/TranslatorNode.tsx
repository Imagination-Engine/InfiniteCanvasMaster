...
const LANGUAGES = [
  ...
  "Turkish", "Vietnamese", "Thai", "Ukrainian", "Greek", "Hebrew", "English"
].sort();
...
export default function TranslatorNode({ id, data, selected }: NodeProps) {
  ...
  const result = typeof nodeData.outputs?.result === "string" ? nodeData.outputs.result : "";
  const detectedLanguage = typeof nodeData.outputs?.detectedLanguage === "string" ? nodeData.outputs.detectedLanguage : "";

  return (
    <div className={`min-w-[300px] flex h-full w-full min-h-[220px] flex-col rounded-[24px] border bg-brand-bg-surface/90 backdrop-blur-2xl p-4 text-brand-text-body shadow-2xl transition-all duration-300 ${selected ? "border-brand-purple shadow-[0_0_30px_-5px_rgba(123,92,234,0.3)]" : "border-brand-purple/40 shadow-inner"}`}>
      <NodeResizer 
        isVisible={selected} 
        minWidth={300} 
        minHeight={220}
      ...
      <div className="space-y-4">
        <label className="block space-y-1.5">
          <span className="block text-[9px] font-black uppercase tracking-widest text-brand-text-muted px-0.5">targetLanguage</span>
          <div className="relative group/input">
            <select
              value={targetLanguage}
              onChange={(event) => updateData({ inputs: { targetLanguage: event.target.value } })}
            ...
            >
              {LANGUAGES.map((lang) => (
                <option key={lang} value={lang} className="bg-brand-bg-surface text-white">{lang}</option>
              ))}
            </select>
        ...
        <label className="block space-y-1.5">
          <span className="block text-[9px] font-black uppercase tracking-widest text-brand-text-muted px-0.5">additionalInstructions</span>
          <div className="relative group/input">
            <textarea
      ...
      {detectedLanguage && (
        <div className="mt-4 flex items-center gap-2.5 px-1 text-[10px] font-bold uppercase tracking-widest text-brand-text-muted">
          <Languages size={12} className="text-brand-purple" />
          Detected:
          <span className="rounded-lg bg-brand-purple/10 border border-brand-purple/20 px-2.5 py-1 text-brand-purple">
            {detectedLanguage}
          </span>
        </div>
      )}
      ...
      {result && (
        <div className="mt-4 rounded-2xl border border-white/5 bg-brand-bg-page/80 p-4 shadow-inner group/out">
          ...
          <div className="text-[11px] font-medium leading-relaxed text-brand-text-body whitespace-pre-wrap">{result}</div>
        </div>
      )}
