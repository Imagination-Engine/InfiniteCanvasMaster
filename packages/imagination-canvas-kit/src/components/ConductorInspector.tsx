...
import { X, Settings, Zap, Globe, Bot, Clock, GitBranch, Database, AlertTriangle, Repeat, Router, Send, FileText, Mail, Sheet } from "lucide-react";
...
const CONDUCTOR_FIELDS: Record<string, FieldDef[]> = {
  "conductor.webhook": [
    { key: "path", label: "Webhook Path", type: "text", placeholder: "/hooks/my-trigger" },
  ],
  "trigger.webhook": [
    { key: "path", label: "Webhook Path", type: "text", placeholder: "/hooks/my-trigger" },
  ],
  ...
  "conductor.agent": [
    { key: "instructions", label: "Agent Instructions", type: "textarea", placeholder: "Describe what this agent should do..." },
    { key: "model", label: "Model", type: "select", options: ["gemini-2.5-pro", "gemini-2.5-flash", "gemini-2.0-flash"] },
  ],
  "conductor.saas": [
    { key: "provider", label: "Provider", type: "select", options: ["slack", "notion", "gmail", "discord", "sheets"] },
    { key: "action", label: "Action", type: "text", placeholder: "sendMessage" },
    { key: "params", label: "Parameters (JSON)", type: "textarea", placeholder: '{"channel": "#general"}' },
  ],
  "conductor.webFetch": [
    { key: "url", label: "URL", type: "text", placeholder: "https://api.example.com/data" },
    { key: "method", label: "Method", type: "select", options: ["GET", "POST", "PUT", "DELETE"] },
    { key: "body", label: "Request Body", type: "textarea", placeholder: "Optional JSON body" },
  ],
  "conductor.slackPost": [
    { key: "channel", label: "Channel", type: "text", placeholder: "#general" },
    { key: "message", label: "Message Template", type: "textarea", placeholder: "Hello from the workflow!" },
  ],
  "conductor.notionCreate": [
    { key: "databaseId", label: "Database ID", type: "text", placeholder: "abc123..." },
    { key: "properties", label: "Properties (JSON)", type: "textarea", placeholder: '{"Name": {"title": [{"text": {"content": "New Page"}}]}}' },
  ],
  "conductor.if": [
    { key: "condition", label: "Condition", type: "text", placeholder: 'data.status === "active"' },
  ],
  "conductor.router": [
    { key: "condition", label: "Routing Rule", type: "text", placeholder: 'data.status === "active"' },
  ],
  "conductor.forEach": [
    { key: "collection", label: "Collection Path", type: "text", placeholder: "$previous.items" },
    { key: "loopTarget", label: "Loop Target", type: "text", placeholder: "processItem" },
  ],
  "conductor.delay": [
  ...
  "conductor.errorBoundary": [
    { key: "node", label: "Target Node ID", type: "text", placeholder: "node-to-protect" },
  ],
  "conductor.subGraph": [
    { key: "graphId", label: "Sub-Graph ID", type: "text", placeholder: "graph-abc123" },
  ],
};
...
const PREFIX_FIELDS: Record<string, FieldDef[]> = {
  "discord.": [
    ...
    { key: "message", label: "Message", type: "textarea", placeholder: "Hello Discord!" },
  ],
  "slack.": [
    { key: "channel", label: "Channel", type: "text", placeholder: "#general" },
    { key: "message", label: "Message", type: "textarea", placeholder: "Hello Slack!" },
  ],
  "gmail.": [
    { key: "to", label: "To Email", type: "text", placeholder: "user@example.com" },
  ...
  "sheets.": [
    { key: "spreadsheetId", label: "Spreadsheet ID", type: "text", placeholder: "1BxiMVs0..." },
  ...
  "trello.": [
    ...
    { key: "cardName", label: "Card Name", type: "text", placeholder: "New Task" },
  ],
  "zoom.": [
    { key: "topic", label: "Meeting Topic", type: "text", placeholder: "Weekly Sync" },
    { key: "when", label: "When", type: "text", placeholder: "next Tuesday 2pm" },
  ],
  "calendly.": [
    { key: "eventType", label: "Event Type", type: "text", placeholder: "30-min-intro" },
  ],
};
...
export const ConductorInspector: React.FC = () => {
  ...
  const handleFieldChange = useCallback(
    (fieldKey: string, value: string | number | boolean) => {
      ...
      updateObject(selectedId, {
        metadata: {
          ...object.metadata,
          config: { ...(object.metadata?.config || {}), [fieldKey]: value },
          inputs: { ...(object.metadata?.inputs || {}), [fieldKey]: value },
        },
      });
    },
  ...
  const getFieldValue = useCallback(
    (fieldKey: string): string => {
      if (!object) return "";
      const config = object.metadata?.config as Record<string, unknown> | undefined;
      const inputs = object.metadata?.inputs as Record<string, unknown> | undefined;
      const meta = object.metadata as Record<string, unknown> | undefined;
      const val = config?.[fieldKey] ?? inputs?.[fieldKey] ?? meta?.[fieldKey] ?? "";
      return String(val);
    },
    [object],
  );
  ...
  const [activeTab, setActiveTab] = React.useState<"configure" | "assistant">("configure");
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/5 shrink-0">
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              <div className="overflow-hidden">
                <h3 className="text-sm font-bold text-white truncate">{displayLabel}</h3>
          ...
          <div className="flex px-4 border-b border-white/5 bg-white/[0.02] shrink-0">
            <button
              ...
              className={`flex-1 py-3 text-center text-[10px] font-black uppercase tracking-[0.2em] transition-all relative outline-none ${
                activeTab === "configure" ? "text-white" : "text-white/40 hover:text-white/70"
              }`}
            >
              Configure
              {activeTab === "configure" && (
            ...
            <button
              ...
              className={`flex-1 py-3 text-center text-[10px] font-black uppercase tracking-[0.2em] transition-all relative outline-none flex items-center justify-center gap-1.5 ${
                activeTab === "assistant" ? "text-white" : "text-white/40 hover:text-white/70"
              }`}
            >
              <Bot size={11} className={activeTab === "assistant" ? "text-brand-cyan" : "text-white/40"} />
              Block AI
              {activeTab === "assistant" && (
                <motion.div
                  layoutId="active-tab-indicator"
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
              {fields.length > 0 ? (
                <div className="space-y-3">
                  <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-brand-purple">Configuration</p>
                    ...
                    {fields.map((field) => (
                      <div key={field.key} className="space-y-1.5">
                        ...
                        {field.type === "select" && (
                          ...
                          >
                            <option value="" className="bg-[#111128]">Select...</option>
                            {(field.options || []).map((opt) => (
                              <option key={opt} value={opt} className="bg-[#111128]">{opt}</option>
                            ))}
                          </select>
                        )}
                      </div>
                ...
                <div className="space-y-3">
                  <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-white/40">Block Properties</p>
                  <div className="space-y-3.5">
                    {object.metadata &&
                      Object.entries(object.metadata)
                        .filter(([key]) => !["label", "description", "category", "role", "runtime", "capabilities"].includes(key))
                        .map(([key, val]) => {
                          if (typeof val === "object" && val !== null) {
                            return (
                              <div key={key} className="space-y-1.5">
                                <label className="text-[9px] font-bold uppercase tracking-[0.15em] text-white/40 block">{key}</label>
                                <textarea
                                  value={JSON.stringify(val, null, 2)}
                                  onChange={(e) => {
                                    try { handleFieldChange(key, e.target.value); } catch {}
                                  }}
                                  rows={3}
              ...
              <div className="pt-5 border-t border-white/5 space-y-3">
                <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-rose-400">Error Handling</p>
                <div className="flex items-start justify-between p-4 rounded-2xl bg-white/5 border border-white/10 hover:border-brand-purple/30 transition-all gap-4">
                  <div className="space-y-1 pr-2">
                    <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-white block">Continue on Fail</span>
                    <span className="text-[9px] text-white/40 leading-relaxed block">Keep executing downstream nodes even if this node encounters an error.</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      const current = object.metadata?.config?.continueOnFail === true;
                    ...
                    className={`w-9 h-5 rounded-full p-0.5 transition-colors duration-200 outline-none shrink-0 mt-0.5 ${
                      object.metadata?.config?.continueOnFail === true ? "bg-gradient-to-r from-brand-purple to-brand-cyan" : "bg-white/10"
                    }`}
                  >
                    <div
                      className={`w-4 h-4 rounded-full bg-white shadow-md transform transition-transform duration-200 ${
                        object.metadata?.config?.continueOnFail === true ? "translate-x-4" : "translate-x-0"
                      }`}
                    />
                  </button>
                </div>
              ...
              {object.metadata?.description && (
                <div className="pt-5 border-t border-white/5 space-y-3">
                  <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-white/40">Description</p>
                  <p className="text-[11px] text-white/50 leading-relaxed">{String(object.metadata.description)}</p>
                </div>
              )}
            </div>
          ) : (
            <BlockAssistantPanel selectedId={selectedId!} object={object!} onClose={clearSelection} />
          )}

          {/* Footer */}
          <div className="px-5 py-3 border-t border-white/5 shrink-0">
            <p className="text-[8px] text-white/20 text-center font-bold uppercase tracking-[0.15em]">Changes save automatically</p>
          </div>
        </motion.div>
      )}
    </AnimatePresence>
