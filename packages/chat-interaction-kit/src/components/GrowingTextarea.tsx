...
interface GrowingTextareaProps extends React.TextareaHTMLAttributes<HTMLTextAreaElement> {
  ...
  fileAccept?: string;
  /** Renders inside the composer chrome (e.g. staged image thumbnails). */
  composerPreview?: React.ReactNode;
...
>(
  (
    ...
    return (
      <div className="flex flex-col w-full bg-white/5 border border-white/10 rounded-2xl focus-within:border-brand-cyan/50 focus-within:bg-white/10 transition-all overflow-visible">
        {composerPreview ? (
          <div className="p-3 overflow-x-auto">{composerPreview}</div>
        ) : null}
