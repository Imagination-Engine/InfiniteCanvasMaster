...
export const nodesRelations = relations(nodes, ({ one, many }) => ({
  canvas: one(canvases, { fields: [nodes.canvasId], references: [canvases.id] }),
  outgoingEdges: many(edges, { relationName: 'sourceEdges' }),
  incomingEdges: many(edges, { relationName: 'targetEdges' }),
}));

export const edgesRelations = relations(edges, ({ one }) => ({
  canvas: one(canvases, { fields: [edges.canvasId], references: [canvases.id] }),
  source: one(nodes, { fields: [edges.sourceId], references: [nodes.id], relationName: 'sourceEdges' }),
  target: one(nodes, { fields: [edges.targetId], references: [nodes.id], relationName: 'targetEdges' }),
}));
