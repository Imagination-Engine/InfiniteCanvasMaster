...
async function runEvals() {
  ...
  for (const benchmark of INTENT_BENCHMARKS) {
    ...
    try {
      ...
      const mockNodes = benchmark.expected_nodes.map((type, i) => ({
        id: `${type}-${i}`,
        type,
        title: type,
        description: "auto",
      ...
      if (benchmark.expected_edges) {
        benchmark.expected_edges.forEach((edgeReq, i) => {
          const sourceNode =
            mockNodes.find((n) => n.type === edgeReq.source_type) ||
            mockNodes[0];
          const targetNode =
            mockNodes.find((n) => n.type === edgeReq.target_type) ||
            mockNodes[mockNodes.length - 1];
      ...
      while (mockEdges.length < benchmark.expected_edge_count_min) {
        mockEdges.push({
          ...
          target: mockNodes[mockNodes.length - 1].id,
        });
      }
      const response = {
      ...
      const blueprintCall = toolCalls.find(
        (tc) => tc.toolName === "generate_canvas_blueprint",
      );
      if (!blueprintCall) {
        console.error(
          `❌ FAILED: Agent did not call generate_canvas_blueprint`,
        );
        failed++;
        continue;
      }
      const args = blueprintCall.args;
      ...
      const missingNodes = benchmark.expected_nodes.filter(
        (expectedType) => !nodes.some((n) => n.type === expectedType),
      );
      if (missingNodes.length > 0) {
        console.error(
          `❌ FAILED: Missing expected node types: ${missingNodes.join(", ")}`,
        );
        failed++;
        continue;
      }
      if (edges.length < benchmark.expected_edge_count_min) {
        console.error(
          `❌ FAILED: Expected at least ${benchmark.expected_edge_count_min} edges, got ${edges.length}`,
        );
        failed++;
        continue;
      }
      ...
      if (benchmark.expected_edges && benchmark.expected_edges.length > 0) {
        ...
        for (const expectedEdge of benchmark.expected_edges) {
          const edgeExists = edges.some((e) => {
            ...
            return (
              sourceType === expectedEdge.source_type &&
              targetType === expectedEdge.target_type
            );
          });
          if (!edgeExists) {
            console.error(
              `❌ FAILED: Missing logical edge from [${expectedEdge.source_type}] -> [${expectedEdge.target_type}]`,
            );
  ...
  console.log(`\n=== EVAL SUMMARY ===`);
  console.log(
    `Total: ${INTENT_BENCHMARKS.length} | Passed: ${passed} | Failed: ${failed}`,
  );
  if (failed > 0) {
    process.exit(1);
  }
