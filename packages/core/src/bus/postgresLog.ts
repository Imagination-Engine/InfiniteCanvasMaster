...
const { db, a2aEventLogs } = dbModule as any;

export class PostgresEventLog implements A2AEventLog {
  ...
  async query(query: A2AReplayQuery): Promise<BalnceEnvelope[]> {
    try {
      ...
      return results.map((r: any) => r.envelope as BalnceEnvelope);
    } catch (error) {
  ...
  async clear(query?: A2AReplayQuery): Promise<void> {
