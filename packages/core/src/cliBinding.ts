...
export const realShellExecutor: ShellExecutor = {
  execSync,
};

export async function executeIemCommand(
  command: string,
  args: string[],
  executor: ShellExecutor = realShellExecutor,
) {
  ...
  try {
    ...
    const output = executor.execSync(
      `npx tsx ${cliPath} ${command} ${args.join(" ")}`,
      { encoding: "utf-8" },
    );
