...
const VERSION = "v1";
export function deriveKey(masterKey, service) {
...
export function encrypt(plaintext, masterKey, service) {
    ...
    if (serviceKey.length !== 32)
        throw new Error("Invalid key length derived");
...
export function decrypt(encryptedString, masterKey) {
    ...
    const serviceKeyHex = deriveKey(masterKey, service);
    const serviceKey = Buffer.from(serviceKeyHex, "hex").subarray(0, 32);
    const decipher = crypto.createDecipheriv(ALGORITHM, serviceKey, Buffer.from(ivHex, "hex"));
